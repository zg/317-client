...
public final class BZip2Decompressor {
	...
	/*
	 * http://svn.apache.org/repos/asf/labs/axmake/trunk/src/libuc++/srclib/bzip2/huffman.c
	 */
	private static void createDecodeTables(int[] limit, int[] base, int[] perm, byte[] length, int minLength, int maxLength,
			int alphabetSize) {
		int pp = 0;
	...
	private static void decompress(BZip2DecompressionState state) {
		...
		if (BZip2DecompressionState.tt == null) {// Because this isn't small decompress
		...
		while (flag19) {
			...
			for (int i = 0; i < swapCount; i++) {
				...
				/*
				 * zero-terminated (see above) bit runs (0..62) of MTF'ed Huffman table (*selectors_used)
				 */
	...
	private static void finish(BZip2DecompressionState state) { // TODO lame name
		...
		outer: do {
			...
			if (stateOutLen > 0) {
				do {
					...
					/*
					 * In the actual implementation it updates the BZ CRC here, but JaGex's doesn't do this.
					 */
					stateOutLen--;
					csNextOut++;
					length--;
